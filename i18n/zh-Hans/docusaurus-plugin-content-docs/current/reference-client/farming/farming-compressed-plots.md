...
- `use_gpu_harvesting`: Set to `true` to enable harvesting with a GPU. <code>use_gpu_harvesting</code>: Set to <code>true</code> to enable harvesting with a GPU. Note that in order to use this setting, your harvester must have an NVIDIA GPU with CUDA capability 5.2 and up, with at least 8GB of vRAM.
