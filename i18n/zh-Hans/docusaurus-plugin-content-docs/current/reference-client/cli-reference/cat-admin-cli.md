...
Just as with standard CATs, the CR-CAT has been minted and sent to its destination address. <code>Asset ID</code> can now be added in the destination wallet. `Asset ID` can now be added in the destination wallet.

In this case, the destination wallet is the holder of a VC with the proof required to hold this CR-CAT. To verify this, run the <code>vcs get</code> CLI command: To verify this, run the `vcs get` CLI command:
