...
Offer sniping, as laid out in the example, isn't possible in the default mempool. This is intentional, and by design -- the default mempool's [superset rule](#super-set-rule) prevents it from happening. This rule requires a strict superset in order for RBF to be applied. In the example, Bob's attempt at sniping would *replace* Alice's original spend, so the mempool will reject it.
