...
Fast forward is a solution to these issues. The mempool can effectively *rebase* singleton spends on the fly, so if you spend the same vault multiple times it won't be rejected by the mempool (provided the spend itself is eligible for the fast forward rules).
