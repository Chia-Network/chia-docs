...
信息
CAT1 将在2311760区块高度结束其生命周期，也就是UTC时间 2022年7月26日 17:00。
更多关于结束生命周期的信息可以参考CAT2 介绍及FAQ
本文档将向 CAT1 发行人展示如何：
1. 生成其 CAT1 状态的快照。 这将显示Puzzle哈希（地址）和每个单独代币在生命周期结束高度的值
2. 从快照中导出他们所需的数据
...
注意：
我们推荐你跟着本节内容来生成你的CAT1快照。请注意这一过程肯能会需要40多个小时来完成。为了方便您，我们会发布一个快照供您使用。这个快照应该与你自己生成的完全相同。
 本节将向您展示如何安装 [CAT-addresses 工具](https://github.com/Chia-Network/CAT-addresses)并使用它来获取特定 CAT1 的每个硬币的puzzle哈希值（地址）和价值的快照。 它甚至可以获取 Chia 区块链上所有 CAT1 的信息。
...
3. 使用本工具需要设置以下环境变量:
    ...
    3. `START_HEIGHT` - 开始创建快照的起始高度（默认值：`0`）。 如果您尝试获取您的CAT的所有记录，建议的起始高度为 `1146800`，即在 CAT1上线之后。

    4. `TARGET_HEIGHT` - 结束快照的区块链高度（无默认值 - 必须设置）。 建议高度为 `2311760`，这是 CAT1 有效的最后一个区块。

    :::caution
    **注意**<br /> 
    此过程可能需要40多个小时才能完成。 您可能希望先将`TARGET_HEIGHT`设置为`1146900`来进行测试。这将只从100个块中提取数据，这应该只需要几秒钟。
...
3. 为您的 CAT 生成一个包含所有内层Puzzle哈希值和金额的 CSV 文件：
