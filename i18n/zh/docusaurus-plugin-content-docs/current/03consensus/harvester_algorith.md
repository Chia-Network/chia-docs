...
1. The harvester receives a signage point, and computes the plot filter:
  ...
  If the resulting bits start with 9 zeroes, then the plot passes the filter. This does not require disk access, since the plot_ids are stored in memory. This does not require disk access, since the plot_ids are stored in memory.
