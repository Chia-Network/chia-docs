...
1. Click "CREATE A NEW PRIVATE KEY" <figure> ![Create a new key](/img/custody/create_new_key.png) </figure>
2. You are recommended to copy the mnemonic (on paper or on your computer) for later recovery
3. Click "NEXT" <figure> ![Copy the mnemonic seed and click next](/img/custody/copy_seed.png) </figure>
4. After a few seconds, you will be taken to your new wallet. Copy the number next to `Wallet` (on paper or on your computer). This is your wallet's fingerprint. You will need this number in a later step. Click `RECEIVE` <figure> ![Copy the wallet fingerprint](/img/custody/fingerprint.png) </figure>
5. You will be shown your wallet's primary receive address. Click the button to copy this address to your computer's clipboard. (It doesn't matter if it starts with XCH or TXCH) <figure> ![Copy the receive address](/img/custody/receive_address.png) </figure>
...
<figure>
  <img
    src="/img/custody/address_to_puzzle_hash.png"
    alt="Address to puzzle hash converter"
  />
</figure>

Copy this to your computer's clipboard, to be used in the next step.

:::info What Chia calls a "puzzle hash" is the tree hash of the puzzle that can be run with your public/private key pair. This puzzle hash can then be encoded into a wallet address with any prefix, such as `xch` or `txch`. You have done the opposite in this step -- you have _decoded_ a wallet address back into the original puzzle hash. The resulting puzzle hash will be the same, regardless of the prefix of the wallet address. For this reason, it didn't matter if your wallet address started with `xch` or `txch`. :::
...
<figure>
  <img
    src="/img/custody/workflow_create.png"
    alt="Workflow to create a testnet"
  />
</figure>
...
<figure>
  <img
    src="/img/custody/workflow_request_success.png"
    alt="Workflow request success"
  />
</figure>
...
:::info If you don't already have the `git` CLI tool installed, [follow these instructions](https://git-scm.com/book/en/v2/Getting-Started-Installing-Git) to install it :::
...
Run `chia show -s`. This will show the status of the network. The timelord and farmer required to run the network were created and started automatically when you created the branch in the previous step.
...
Your custom testnet has created a new folder called `~\.chia\ictest-<your name>`. However, the custody tool will automatically use a version of Chia that expects this folder to be called `~\.chia\mainnet`. To avoid issues later, do one of the following:
