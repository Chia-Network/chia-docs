...
正如您在上面的代码示例中看到的那样，一个支出包是一组硬币支出，其中每个硬币支出包括正在支出的硬币、拼图程序和解决方案程序。每个解决方案都被传递到每个谜题中，并通过 clvm 运行，在那里它输出条件。其中两个条件——“AGG_SIG_ME”和“AGG_SIG_UNSAFE”——要求存在签名才能使支出有效。

通常，在进行交易时，用户会合并多次消费，以实现交易的预期价值。 用户还可以组合代表 [CATs](https://chialisp.com/docs/puzzles/cats) (Chia 资产代币) 的硬币，并在同一个支出包中发送不同类型的价值。
...
在比特币中，每笔支出都有自己的一个或多个签名，每个“（公钥，消息）”组合一个。 然后，每个签名`s_i`都通过一个签名验证算法：`V(s, m, pk) -> T/F`，带有消息`m`和公钥`pk`，并且当且仅当 `V` 返回 true 时。

在 Chia 中，使用 BLS 签名。 这些签名可以组合（添加）在一起以生成与原始签名相同大小的聚合签名。 假设我们有三个消费包，每个包都有自己的一对：
...
这允许农民将所有三个消费包合并为一个消费包，只有一个签名。这意味着更少的数据传输和存储在链和磁盘上。
...
假设一个农民想要将 1000 个消费包包含在一个块中。 首先，他们可以将所有的支出包合并为一个，然后他们可以制作区块。 每个区块的所有支出都只有一个签名。 验证和存储这个区块的全节点不需要知道哪些花费与哪些其他花费捆绑在一起的原始信息。
...
让我们来看看图像中的不同组件。首先，假设 Alice 想向 Bob 发送 13 个 XCH。 Alice 查看她的硬币数据库，并选择 3 个未使用的硬币（A、B 和 C），它们加起来至少有 13 个 XCH。这些硬币中的每一个都有一个相关的 CLVM 谜题，其中有一个公钥编码。让我们将这些公钥表示为`pkA`、`pkB`和`pkC`。 Alice 需要为每笔支出生成谜题和解决方案，以创建支出包。

每个谜题，当与解决方案一起运行时，会返回一个`AGG_SIG`条件（AGG_SIG_ME 或 AGG_SIG_UNSAFE）。这意味着需要来自相应公钥的签名，以便此支出有效。
...
也就是说，获得 Alice 的支出包的攻击者无法拆开支出包以仅花费其中一个硬币。当 Alice 将此支出包发送到 Chia 网络时，其他全节点将运行 CLVM 程序，收集所有`AGG_SIG`条件，然后使用支出包中提供的聚合签名对其进行验证。

硬币 A 的第一个谜题也返回两个`CREATE_COIN`条件。这意味着必须将两个硬币添加到硬币数据库中才能使支出 A 有效：
1. 币 D 是给 Bob 的（拼图哈希 0x1b54f 和 13 XCH）。
2. 币 E 是 Alice 的零钱。她花了14个XCH，却只想送 13 个 XCH 给 Bob。每个硬币的价值都必须完全花掉，所以 Alice 需要向自己发送 1 个 XCH 作为找零。

请注意，硬币 E 的拼图哈希与硬币 A 的拼图哈希相同。拼图哈希可以重复使用。当花费硬币 E 时，Alice 将使用与之前相同的密钥进行签名，但很可能会使用不同的消息来支付给另一个接收者。

另请注意，只有第一次花费是创建硬币。这是合并支出的正常方式，因为每个代币必须只有一个父代币。花费 A 本身是无效的，因为它创造的价值 (14 XCH) 比花费的 (5 XCH) 多。但是，当与花费 B 和 C 结合使用时，花费才有效。

全节点接收、验证支出包并将其存储在内存中。但是，在创建新区块时，农业节点会合并来自不同用户的许多支出包。这会创建一个带有一个签名的大花束。仅查看区块时，并不总是清楚最初将哪些支出捆绑在一起。但是，我们可以看到整个区块的净增减。
...
只有在花费硬币时才会显示谜题； 之后，谜题会保存在链上以备记录。

因此，完整节点存储区块链的历史，其中包括所有已用硬币的所有谜题和解决方案。 完整节点还存储未花费硬币的列表，其中仅包含拼图哈希，而不包含拼图。

用户有责任记住和存储他们自己的谜题以花费他们的硬币。 通常这些是由钱包软件根据模板即时重新生成的。
