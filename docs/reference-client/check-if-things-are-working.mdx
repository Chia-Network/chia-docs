...
This doc assumes you know how to use the CLI. Using the CLI is the best way to troubleshoot (and to do everything Chia too). The [Quick Start Guide](/quick-start-guide) and [CLI Commands Reference](/cli-commands) have useful info to get you familiar with the CLI.
...
You can find the documentation for the `check` command on the [CLI Commands Reference - check](/cli-commands#check) page
