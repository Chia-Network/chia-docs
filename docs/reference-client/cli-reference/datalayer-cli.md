...
2. If the fee option is not explicitly specified, then the `data_layer:fee` setting in `~/.chia/mainnet/config/config.yaml` will be used. By default, this is set to 1 billion mojos (0.001 XCH). For comprehensive configuration options, see the [Configuration Guide](/reference-client/install-and-setup/configuration).
