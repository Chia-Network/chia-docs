...
- **No requirements to participate**, like ID, bank account, or nationality. Anyone with an internet connection can participate in the new crypto economy.
...
First, we need a secure way to send transactions to many servers, instead of one. Let's assume that there are 1000 servers across the world, instead of just one, and these servers send transaction information of users to each other.
...
Each server running in this decentralized system can accept a transaction, which includes the ID of the
coin that is being sent, the recipient information, and the signature.
...
### Double Spending
However, signatures are not enough, because of an issue called the "double spend problem". Of the 1000 servers, let's say 500 are in Asia and 500 are in America. An attacker, Bob, sends two transactions that spend the same coin, to two servers, at the same time: one in Asia and one in America. However, those transactions send the money to different recipients. 
...
We could instead base the system on "one computer, one vote", counting each IP address as a "computer". However, it is trivial to buy new IP addresses, or to change addresses using a VPN or a proxy server. An attacker could even create millions of fake IP addresses. The attacker would gain control of the network once they own 51% of the IP addresses. At this point, they could decide transaction ordering and content. Again, the system becomes centralized, and possibly compromised.
...
Each block also has a hash pointer to the previous block. This means the hash of the contents of the previous block are included in the current block. If an attacker could find an alternative valid proof for a historical block, the proof would then change that block's hash, which would invalidate the next block. If the attacker wanted to change a block that occurred 10 blocks in the past, he or she would therefore need to re-do the proof of work for 10 blocks. As long as the rest of the network, combined, could create blocks at the same rate or fast, the attacker would never be able to create a chain that is longer than the legitimate chain.
