---
title: DataLayer CLI Reference
sidebar_label: 13.5 DataLayer CLI Reference
sidebar_position: 5
---
...
This page includes a comprehensive list of Chia's Command Line Interface commands for interacting with the DataLayer. See also:

- [RPC](/docs/12rpcs/datalayer_rpcs 'DataLayer RPC API') commands for interacting with the DataLayer
...
| -a            | --amount        | INTEGER | True     | The amount (in mojos) that will be locked into the mirror while it exists. In theory, a system could prioritize mirrors according to how much was spent to create them. The `amount` will be returned to the creator when the mirror is deleted. Minimum `amount` is 0 |
| -u            | --url           | TEXT    | False    | A URL where the mirror will reside. Can be repeated to add multiple URLs in the same command                                              |
| -m            | --fee           | TEXT    | False    | Set the fee for the transaction, in XCH                                                                                                   |
...
| Short Command | Long Command    | Type    | Required | Description                                                                                                 |
| :------------ | :-------------- | :------ | :------- | :---------------------------------------------------------------------------------------------------------- |
| -i            | --id            | TEXT    | True     | The coin_id of the mirror to delete (obtainable from the [get_mirrors](#get_mirrors) command)               |
| -m            | --fee           | TEXT    | False    | Set the fee for the transaction, in XCH                                                                     |
| -dp           | --data-rpc-port | INTEGER | False    | Set the port where the DataLayer is hosting the RPC interface. See rpc_port under data_layer in config.yaml |
| -h            | --help          | None    | False    | Show a help message and exit                                                                                |
...
```bash
chia data delete_mirror -i b5756487c17fe3a2628e45a9d3d42e89231af718bb1735e6c8441e07ec005f9d -m 0.000001
```
