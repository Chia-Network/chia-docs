...
- **Op Constraints/Conditions** - Constraints are returned by the puzzle when it’s passed the solution. If all of the returned conditions are met then a transaction is valid. All conditions are documented on the [chialisp site](https://chialisp.com/conditions/).
