...
This tutorial will show you how to set up and use Chia's custody tool.
...
- A synced [Chia node](https://github.com/Chia-Network/chia-blockchain/wiki/INSTALL "Chia installation instructions") (mainnet, testnet, or [simulator](https://devs.chia.net/guides/simulator-user-guide "Simulator user guide"))
- A synced wallet with some TXCH
...
An HSM is a physical/offline security solution used for creating and using cryptographic signatures. While testing the custody tool, it is OK to run these commands from a normal computer. However, for maximum security while running the custody tool on Chia's mainnet, you are recommeded to use an HSM.
...
- Because `n` will be 3, we will create 3 keys now. `m` and `n` will be used later when setting up the singleton.

When creating and using these keys on mainnet, you should have an HSM for each key. Signing then requires copying an unsigned spend bundle to each individual computer that holds a key. This process is explained in detail later in the guide.
...
- The specific time values for this example were chosen for testing purposes only. In a mainnet custody solution, the values are typically measured in months rather than minutes
...
This command includes an optional `--configuration` flag to speficy the name and location of the configuration file. By default, the command will look in `./Configuration (needs derivation).txt`.
...
Aside from the fees, the `launch_singleton` command includes an optional `--configuration` flag to speficy the name and location of the configuration file. By default, the command will look in `./Configuration (awaiting launch).txt`.

Because this command modifies the blockchain, a synced wallet is required to run it. To be sure your wallet is still synced, run:
...
To get the status of the transaction, run the command indicated at the end out the output. If you added a fee, it should be added to the blockchain within a few minutes. The output of this command should show `Status: Confirmed`. For example:

```bash
~/internal-custody/keys_and_sb$ chia wallet get_transaction -f 394934909 -tx 0x2728390c1b88a1f1b4db8afd314b099182f9fc78d6046877531d72d401d8aaf2
...
:::info
About the [examine_spend](/custody-tool-cli#examine_spend) command:
...
At some point in the future, this step will also assign human readable metadata so that people can know what they're signing before they enter an HSM.
