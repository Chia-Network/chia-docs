...
:::warning
In order to use this tool, you are required to run a fully synced Chia node. This node must be running version 1.5 or greater, and it must have been [installed from source](https://github.com/Chia-Network/chia-blockchain/wiki/INSTALL#install-from-source) (not from a binary installer). Please ensure that you have satisfied these requirements before continuing.
:::

1. Open a terminal window and ensure you have direct access to the `chia` command:
