...
The last step is to actually send the CAT2 tokens to the correct locations (AKA *"unwind the bag"*). Before sending all of the tokens, you can run a test by sending the correct number of tokens to a single address.
...
4. Run the `unwind_the_bag` command to send the appropriate amount to that puzzle hash. Be sure to run this command from the wallet that has the appropriate funds in it.
    ...
    For this example, the command to unwind the bag is:
...
6. The final step is to unwind the bag for the entire set of coins. This will send coins to each appropriate wallet. The only difference between this command and the last one is that you don't use the `--unwind-target-puzzle-hash` flag. 
...
8. At this point, you can navigate to [taildatabase.com](https://www.taildatabase.com/) and register your new CAT2 token, using the Asset ID obtained from the final step of the Secure the Bag section.