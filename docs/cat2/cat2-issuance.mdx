...
Before starting this guide, you will need a snapshot CSV, instructions for which can be found in the [Snapshot Generation guide](/docs/cat2/cat2-snapshot).

:::warning
Chia version 1.5.0 or later is a prerequisite for using this guide. If you installed Chia from the `main` branch in order to generate your snapshot, you will need to update to version 1.5.0 or later now.
:::
...
1. Figure out the total number of XCH mojos that were issued for your CAT1. If you already know this number, then you can skip this step.

  Note that this technique will only work if you previously registered your CAT1 with taildatabase.

    - Navigate to [taildatabase.com](https://www.taildatabase.com).
    - Log in using the same user ID and password you used to register your CAT1.
    - Locate your CAT1. We'll use Spacebucks for this example.
...
5. Push the transaction to the network. This will actually create the coin tree (*Secure the Bag*). The arguments are the same as above, with the following two exceptions:

    - `--send-to` – The XCH address from the “Secure the bag root address:” of the above output.
    - `--fee` - The fee in mojos. If the network is busy, then the recommended fee is 20 million mojos.

    The command to run is:

    ```bash
    cats --tail <HEX-FILE> --send-to <ADDRESS> --amount <AMOUNT> --as-bytes --curry <COIN-ID> --fee <AMOUNT>
    ```
...
1. Figure out the total number of XCH mojos that were issued for your CAT1. If you already know this number, then you can skip this step.
  ...
  Note that this technique will only work if you previously registered your CAT1 with taildatabase.
    ...
    - Log in using the same user ID and password you used to register your CAT1.
    - Locate your CAT1. We'll use Spacebucks for this example.
    - You'll see *Supply* (and a number) under the title on the right side of your screen. The number indicates the number of tokens issued. However, you need to multiply this number by 1,000 in order to calculate the number of XCH mojos used for the issuance. In this example, we'll use an issuance of 6 million (6,000,000) tokens, which is equivalent to 6 billion (6,000,000,000) XCH mojos.
    - Click the *Chialisp* button. This will show you the TAIL that was used for issuance. If it was a multi-issuance CAT, it likely used `delegated_tail`. If you are unsure, compare the TAIL shown with the [TAIL in GitHub](https://github.com/Chia-Network/chia-blockchain/blob/main/chia/wallet/puzzles/delegated_tail.clvm). Take note of this TAIL as you will need to input its hex version in a subsequent step (if you used one of our reference TAILs, then you already have a copy of this file).
...
6. Run the same `cdv clsp curry` command from the previous step, but add the `--treehash` flag at the end:
...
8. The final step is to create the coin using the `secure the bag root address` as the target address.
    ...
    The command to run is (Note that you need to include the parentheses):

    ```
    cats --tail <HEX-FILE> --curry <PUBLIC-KEY> --send-to <ADDRESS> --amount <AMOUNT> --as-bytes --solution "(<PUZZLE> ())" --signature <SIGNATURE> --fee <AMOUNT>
    ```
    ...
    The arguments needed are:
      ...
      - `--solution` – The output of the `cdv clsp curry` command from a previous step, in the format `"(<PUZZLE> ())"` (replace `<PUZZLE>` with the output).
...
:::danger
For **Unwind the Bag** you will need a synced full node.
:::
...
The final step is to actually send the CAT2 tokens to the correct locations (AKA *"unwind the bag"*). This will send coins to each appropriate wallet.
...
At this point, you can navigate to [taildatabase.com](https://www.taildatabase.com/), log in or sign up, and update your asset ID for your CAT2 token, using the Asset ID obtained from the final step of the Secure the Bag section.
...
:::info
**The section is optional:** Before unwinding the bag all at once, you can run a test by sending the correct number of tokens to a single address.
:::
...
1. We used Spacebucks in the single-issuance example, so we'll obtain the puzzle hash of a wallet that contains some CAT1 Spacebucks. First, run:
    ...
    :::note
    In our example, the first wallet address contains Spacebucks. However, this may not be the case for you - you should use any address under your control that you know contains some quantity of the CATs in question.
    :::
...
4. Run the `unwind_the_bag` command to send the appropriate amount to that puzzle hash. Be sure to run this command from the wallet that has the appropriate funds in it.
    ...
    :::note
    Each puzzle hash in the bag can only receive one airdrop. When you later unwind the bag for each puzzle hash, any puzzle hashes that have already received an airdrop will be skipped. They will *not* receive a second airdrop.
    :::

5. The puzzle hashes from the CSV file are actually *inner* puzzle hashes, so searching for them on chain using `cdv rpc coinrecords` is more complex than it normally would be. However, you can still verify that the bag was successfully unwound for that puzzle hash by searching for the hint:
