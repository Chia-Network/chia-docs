...
2. Create a directory for custody testing `mkdir internal-custody`

```powershell
PS C:\Users\User\prefarm_test> mkdir internal-custody
```

3. Change to the new dir by running `cd internal-custody`
...
```powershell
(venv) PS C:\Users\User\prefarm_test\keys_and_sb> cic init --directory $(pwd) -wt 600 -pc 1200 -rt 300 -rc 600 -sp 900
...
At this point, the singleton should exist on the blockchain. However, it has not yet been funded -- you've launched the singleton to the chain, but you haven't actually sent the money to it yet. We'll do that in the next step.
...
This tutorial funds the singleton with 10 million XCH (your value may be different).

A few notes about the command to finance the singleton:

- You need to specify the amount in _XCH_ (`-a 10000000` for this tutorial)
...
Even though the `amount left` to be withdrawn will increase on chain, it won't increase locally until a new `sync` is performed. This is by design -- it avoids surprises.
...
The `amount to claim` should now match the amount that you just sent.
...
Singleton:
  - launcher ID: 2dcadfa8dd5c0d079c9a59e7284754921c94b4ed2c77d194fe18f1a73a588b07
  - amount left: 10000000000000000000
  - amount to claim: 0
...
The `amount to claim` is now 0, so the absorb was successful. `amount left` is the original amount, proving that no money was sent.
...
Note that `amount to claim` (unabsorbed funds) is positive. If you send money at this point, it will automatically pull from the unabsorbed funds, and absorb the rest.

To run this test:

1. Run [Test 4](#test-4---payment-pass), using an amount up to the value of `amount to claim` as the payment amount (`-a`)
2. If the payment goes through without issue, then this test passes
...
If we withdraw slightly more than `amount to claim`, we can be assured that the funds were actually withdrawn using a mix of both amount to claim and amount left. At this point, we can run a payment similar to Test 4. See that test if you need more details.

#### Initiate a payment

The amount to send is `amount left` + `amount to claim`.

```powershell
(venv) PS C:\Users\User\prefarm_test\keys_and_sb> cic payment -f initial_absorb.unsigned -pks "1.pk,2.pk" -a 9999999000000000000 -t txch1q03d7grhvat0wrv7cxu6kxjksz0n2qng6ruw6src5642kyxyphrs8k2u2q -ap -at 0
Successfully wrote spend to initial_absorb.unsigned
```
...
Outstanding events:
  PAYMENTS:
- PAYMENT to xch1q03d7grhvat0wrv7cxu6kxjksz0n2qng6ruw6src5642kyxyphrs23d2tn of amount 9999999000000000000 (Ready)
  REKEYS:
```
...
1) PAYMENT to xch1q03d7grhvat0wrv7cxu6kxjksz0n2qng6ruw6src5642kyxyphrs23d2tn of amount 9999999000000000000
...
Chia Wallet:
   -Total Balance:         9999999.0 txch (9999999000000000000 mojo)
   -Pending Total Balance: 9999999.0 txch (9999999000000000000 mojo)
   -Spendable:             9999999.0 txch (9999999000000000000 mojo)
   -Type:                  STANDARD_WALLET
   -Wallet ID:             1
```

The amount transferred was `9 999 999` txch, which was all of the `amount left` and `amount to claim`. This proves that the unabsorbed and absorbed funds were spent together.
