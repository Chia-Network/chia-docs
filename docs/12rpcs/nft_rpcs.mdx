...
| nft_coin_id    | True     | The coin ID of the NFT to transfer. This can be obtained by running the [nft_get_nfts](#nft_get_nfts) RPC and selecting the value of `nft_coin_id` from the output |
...
:::info
For this RPC, `coin_id` can refer to the specific `nft_coin_id` pertaining to this NFT, or to the `launcher_id`, or to the bech32m encoded launcher id
:::
