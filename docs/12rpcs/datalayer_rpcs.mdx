---
title: DataLayer RPCs
sidebar_label: 12.5 DataLayer RPCs
sidebar_position: 5
---
...
This page includes a comprehensive list of Chia's DataLayer RPC API. See also:

- [CLI](/docs/13cli/datalayer_cli 'DataLayer CLI') commands for interacting with the DataLayer
- The [DataLayer user guide](https://devs.chia.net/guides/datalayer-user-guide 'DataLayer user guide')
...
:::info Maximum Key and Value Size

Generally speaking, it is safe to insert keys and values totalling 50 MiB into a single data store. For more info, see the [DataLayer User Guide](http://localhost:3000/guides/datalayer-user-guide#note-on-maximum-key-and-value-lengths)
...
Functionality: Get the root hash and timestamp of a given store ID. If the store is empty, this command will return a root hash consisting of all zeros (see example). In this case, use [get_local_root](#get_local_root) instead
...
Get the root hash of a remote store. This store happens to be empty, so it will return a root hash of all zeros:
...
Functionality: Get the root hashes and timestamps from a list of stores. Note that a root hash consisting of all zeros will be returned for empty stores. Use [get_local_root](#get_local_root) instead
...
For this example, the first store is empty, so it will return a root hash consisting of all zeros. The second store is local, so the root hash will be valid:
...
| amount | INTEGER | True     | The amount (in mojos) that will be locked into the mirror while it exists. In theory, a system could prioritize mirrors according to how much was spent to create them. The `amount` will be returned to the creator when the mirror is deleted. Minimum `amount` is 0 |
| fee    | TEXT    | False    | Set the fee for the transaction, in mojos                                                                        |
...
| Flag      | Type | Required | Description                                                                              |
| :-------- | :--- | :------- | :--------------------------------------------------------------------------------------- |
| coin_id   | TEXT | True     | The `coin_id` of the mirror to delete, obtainable by running [get_mirrors](#get_mirrors) |
| fee       | TEXT | False    | Set the fee for the transaction, in mojos                                                |
...
```json
chia rpc data_layer delete_mirror '{"coin_id":"6d111d5e7e74126cb35fd159c202a9e8f179e3ed2250b73c58804c50719b222d"}'
```
...
```json
chia rpc data_layer delete_mirror '{"coin_id":"0x83a75913f8222eebfbc047d2ab3bf4b92bd5c37f3acaa336a0bb8b3b3168c879"}'
```
...
| fee   | TEXT | True\* (see Description) | Set the Maker fee for the offer, in mojos. This is a transaction fee that will go to a farmer. If no fee is desired, this parameter may be set to `0`. Note that this fee may also be set with the JSON request, as detailed below. In this case, don't set the `fee` RPC parameter |
...
- As noted above, the `fee` is a required Maker fee, which must be specified either in the JSON or as an RPC parameter. It may be set to `0`. This is a transaction fee that will go to a farmer
...
- Change the last line from `"success": true` to `"fee": <fee amount in mojos>` (for example, `"fee": 10000000`). This is a _required_ transaction fee, paid by the Taker. If this fee is not desired, it may be set to `0`. As noted above, this fee may also be specified as an RPC parameter
