...
Enter Chia's bulk minting tool. This tool allows you to mint an arbitrarily large number of NFTs, complete with separate [offer](https://chialisp.com/docs/puzzles/offers) for each of them. Want to mint your new collection of 10,000 PFPs on the fly? You've come to the right place!

:::info

The number of NFTs per spend bundle is hard-coded as 25 in this tool. It may be possible to mint more than 25 NFTs at a time, but doing so will increase the risk of the spend bundle not getting included onto the blockchain, at least not without an exhorbitant fee. 
...
We strongly recommend that you test the bulk minting tool on the testnet or using the [simulator](/guides/simulator-user-guide) before attempting to use it on mainnet. In addition, you will need to run a fully synced node in order to use this tool (this is true for testnet, mainnet and the simulator).
...
1. Clone the [chianft](https://github.com/Chia-Network/chianft "chianft") GitHub repository, which contains the bulk minting tool.

In order to clone this repository, first open a PowerShell (Windows) or terminal (Linux and MacOS) window. Next, create a new directory and run the `git clone` command:
...
1. **IMPORTANT:** Run the following command to set up Chia to use the testnet:
...
11. In order to run the bulk mint tool, you will need to have an NFT wallet that is associated with a DID. You will need to have some TXCH in order to create this wallet. If your requested TXCH has not yet arrived, post your address on the #dev channel on Keybase. Someone might be able to send some to you.
...
The output of this command will be a royalty address and default percentage (in basis points). You can adjust the royalty percentage when creating the spend bundles (Step 3).

A CSV file called `metadata.csv` will be created in the local directory. This file will contain the sample data.
...
3. Create the spend bundles. This command has six inputs:
  ...
  * The name of the output file where the spend bundles will be stored (`output.pk1` in this example)
...
4. Submit the spend bundles created in the output file (output.pk1 in this example). This command has two flags:
    * `-m`: an optional transaction fee, in mojos. This is a fee to be used for inclusion in the blockchain, completely separate from the royalty percentage. This fee will be applied once per spend bundle of 25 NFTs
...
The output of this command will be 100 offer files, which you can then post to a marketplace, send to individual users, etc.
...
1. Generate the factory data for 100 NFTs. The "t" flag indicates we should include a target address in the metadata csv:
...
3. Create the spend bundles. This command has five inputs:
  * `-w`: the wallet ID for the NFT wallet (`3` in this example)
  * `-d`: a boolean indicating whether to use a DID (default is `True`)
  * `-a`: the royalty address for the NFTs (typically this is the artist's address)
  * `-r`: the royalty percentage, in basis points ("300" means 3%)
...
4. Submit the spend bundles created in the output file (output.pk1 in this example). This command has two flags:
    * `-m`: an optional transaction fee, in mojos. This is a fee to be used for inclusion in the blockchain, completely separate from the royalty percentage. This fee will be applied once per spend bundle of 25 NFTs
    * `-o`: set this to create a separate offer file for each NFT, not needed in this example
    
```bash
chianft submit-spend-bundles -m 10 output.pk1
```

As a result of running this command the NFTs will be air-dropped to the correct recipient addresses in batches of 25.

To calculate the approximate amount of time needed for your air drop, divide the total number of NFTs by 25, and multiple the result by 52 (the average number of seconds per transaction block). For example, with a set of `10 000` NFTs, the approximate time needed is

```
10 000 / 25 = 400 * 52 = 20 800 seconds, or a bit less than six hours
```